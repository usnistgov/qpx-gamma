...
std::string LogInverse::to_str_precision(double x, int n) {
...
LogInverse::LogInverse(std::vector<double> coeffs, double center)
  :LogInverse()
{
  xoffset_ = center;
  coeffs_ = coeffs;
...
LogInverse::LogInverse(std::vector<double> &x, std::vector<double> &y, uint16_t degree, double center)
  :LogInverse()
{
...
LogInverse::LogInverse(std::vector<double> &x, std::vector<double> &y,
                       std::vector<uint16_t> &degrees, double center) {
  fit(x, y, degrees, center);
}


void LogInverse::fit(std::vector<double> &x, std::vector<double> &y,
                       std::vector<uint16_t> &degrees, double center) {
  degree_ = -1;
...
double LogInverse::evaluate(double x) {
  double x_adjusted = 1.0 / (x - xoffset_);
  double result = 0.0;
...
/*
LogInverse LogInverse::derivative() {
  LogInverse new_poly;
...
double LogInverse::inverse_evaluate(double y, double e) {
  int i=0;
  double x0=1;
  LogInverse deriv = derivative();
  ...
  {
    PL_WARN <<"Maximum iteration reached in LogInverse inverse evaluation";
    return nan("");
  }
}
*/
...
std::vector<double> LogInverse::evaluate_array(std::vector<double> x) {
  std::vector<double> y;
  for (auto &q : x)
...
std::string LogInverse::to_string() {
  std::stringstream ss;
  ss << "exp(";
  if (degree_ >=0 )
    ss << coeffs_[0];
...
std::string LogInverse::to_markup() {
  std::string calib_eqn = "exp(";
  if (degree_ >= 0)
...
std::string LogInverse::coef_to_string() const{
...
void LogInverse::coef_from_string(std::string coefs) {
