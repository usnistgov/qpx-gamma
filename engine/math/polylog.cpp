...
std::string PolyLog::to_str_precision(double x, int n) {
...
PolyLog::PolyLog(std::vector<double> coeffs, double center)
  :PolyLog()
{
  xoffset_ = center;
  coeffs_ = coeffs;
...
PolyLog::PolyLog(std::vector<double> &x, std::vector<double> &y, uint16_t degree, double center)
  :PolyLog()
{
...
PolyLog::PolyLog(std::vector<double> &x, std::vector<double> &y,
                       std::vector<uint16_t> &degrees, double center) {
  fit(x, y, degrees, center);
}


void PolyLog::fit(std::vector<double> &x, std::vector<double> &y,
                       std::vector<uint16_t> &degrees, double center) {
  degree_ = -1;
...
double PolyLog::evaluate(double x) {
...
double PolyLog::inverse_evaluate(double y, double e) {
  int i=0;
  double x0=1;
  PolyLog deriv = derivative();
  ...
  {
    PL_WARN <<"Maximum iteration reached in PolyLog inverse evaluation";
    return nan("");
  }
}
*/
...
std::vector<double> PolyLog::evaluate_array(std::vector<double> x) {
  std::vector<double> y;
  for (auto &q : x)
...
std::string PolyLog::coef_to_string() const{
...
void PolyLog::coef_from_string(std::string coefs) {
