...
void Peak::reconstruct(FitSettings fs) {
  ...
  if (hypermet_.height_.value.finite() && (hypermet_.height_.value.value() > 0)) {
    center_ = hypermet_.center_.value;
  }
  else
  ...
  if (hypermet_.width_.value.finite())
  {
    double L = hypermet_.center_.value.value() - hypermet_.width_.value.value() * sqrt(log(2));
    double R = hypermet_.center_.value.value() + hypermet_.width_.value.value() * sqrt(log(2));
    double dmax = (hypermet_.width_.value.value() + hypermet_.width_.value.uncertainty()) * sqrt(log(2));
    double dmin = (hypermet_.width_.value.value() - hypermet_.width_.value.uncertainty()) * sqrt(log(2));
    double Lmax = hypermet_.center_.value.value() - dmax;
    double Rmax = hypermet_.center_.value.value() + dmax;
    double Lmin = hypermet_.center_.value.value() - dmin;
    double Rmin = hypermet_.center_.value.value() + dmin;
...
void Peak::to_xml(pugi::xml_node &root) const {
  ...
  if (hypermet_.height_.value.value() > 0)
