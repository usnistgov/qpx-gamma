/*******************************************************************************
 *
 * This software was developed at the National Institute of Standards and
 * Technology (NIST) by employees of the Federal Government in the course
 * of their official duties. Pursuant to title 17 Section 105 of the
 * United States Code, this software is not subject to copyright protection
 * and is in the public domain. NIST assumes no responsibility whatsoever for
 * its use by other parties, and makes no guarantees, expressed or implied,
 * about its quality, reliability, or any other characteristic.
 *
 * This software can be redistributed and/or modified freely provided that
 * any derivative works bear some notice that they are derived from it, and
 * any modified versions bear some notice that they have been modified.
 *
 * Author(s):
 *      Martin Shetty (NIST)
 *
 * Description:
 *      Pixie::Sorter generates spills from list files
 *
 ******************************************************************************/


#include "sorter.h"
#include <utility>
#include "custom_logger.h"

namespace Pixie {

Sorter::~Sorter() {
  if (open_bin_) {
    PL_DBG << "closing bin";
    file_bin_.close();
  }
}

Sorter::Sorter(std::string name_xml)
  : valid_(false)
  , open_bin_(false)
  , file_name_xml_(name_xml)
{
  file_xml_ = fopen(file_name_xml_.c_str(), "r");
  if (file_xml_ == nullptr)
    return;

  tinyxml2::XMLDocument docx;
  docx.LoadFile(file_xml_);

  if ((root_ = docx.FirstChildElement("QpxListData")) == nullptr) {
    fclose(file_xml_);
    return;
  }

  tinyxml2::XMLElement* branch;
  if ((branch = root_->FirstChildElement("BinaryOut")) == nullptr) {
    fclose(file_xml_);
    return;
  }

  tinyxml2::XMLElement* fn;
  if ((fn = branch->FirstChildElement("FileName")) == nullptr) {
    fclose(file_xml_);
    return;
  }

  file_name_bin_ = std::string(fn->GetText());
  file_bin_.open(file_name_bin_, std::ofstream::in | std::ofstream::binary);

  if (!file_bin_.is_open()) {
    fclose(file_xml_);
    return;
  }

  if (!file_bin_.good()) {
    file_bin_.close();
    fclose(file_xml_);
    return;
  }

  tinyxml2::XMLElement* sp = root_->FirstChildElement("StatsUpdate");
  while (sp != nullptr) {
    StatsUpdate su;
    su.from_xml(sp);
    if (!(su == StatsUpdate()))
      spills_.push_back(su);
    sp = dynamic_cast<tinyxml2::XMLElement*>(sp->NextSibling());
  }

  PL_DBG << "found " << spills_.size() << " spills";

  branch = root_->FirstChildElement("RunInfo");
  if (branch != nullptr)
    start_.from_xml(branch);
  branch = dynamic_cast<tinyxml2::XMLElement*>(branch->NextSibling());
  if (branch != nullptr)
    end_.from_xml(branch);

  PL_DBG << "start count = " << start_.total_events;
  PL_DBG << "end count = " << end_.total_events;

  if (spills_.size() > 0)
    valid_ = true;
  else
    return;

  fclose(file_xml_);
  file_bin_.seekg (0, std::ios::beg);
  bin_begin_ = file_bin_.tellg();
  file_bin_.seekg (0, std::ios::end);
  bin_end_ = file_bin_.tellg();
  file_bin_.seekg (0, std::ios::beg);
  PL_DBG << "bin file size is: " << (bin_end_ - bin_begin_) << " bytes";
  open_bin_ = true;
}

Spill Sorter::get_spill() {
  Spill one_spill;
  int coinc = 0;

  if ((spills_.size() > 0) && (file_bin_.tellg() < bin_end_)) {
    PL_DBG << "will produce spill " << spills_.front().spill_number;
    if (spills_.size() == 1)
      one_spill.run = new RunInfo(end_);
    else if (spills_.front().spill_number == 0)
      one_spill.run = new RunInfo(start_);
    one_spill.stats = new StatsUpdate(spills_.front());

    uint64_t count = 0;
    uint16_t chan = 0;
    uint16_t event_entry[5];
    PL_DBG << "will produce no of events " << spills_.front().events_in_spill;
    while ((count < spills_.front().events_in_spill) && (file_bin_.tellg() < bin_end_)) {
      Hit one_event;
      file_bin_.read(reinterpret_cast<char*>(event_entry), 10);
      chan = event_entry[0];
//      PL_DBG << "event chan " << chan;
      one_event.pattern[chan] = 1;
      one_event.buf_time_hi = event_entry[1];
      one_event.buf_time_mi = event_entry[2];
      one_event.buf_time_lo = event_entry[3];
      one_event.energy[chan] = event_entry[4];
      //file_bin_.seekg(10, std::ios::cur);

      if (file_bin_.tellg() < bin_end_) {
        file_bin_.read(reinterpret_cast<char*>(event_entry), 10);
        int32_t dif = event_entry[3] - one_event.buf_time_lo;
        if (abs(dif) < 64) {
          chan = event_entry[0];
          one_event.pattern[chan] = 1;
          one_event.energy[chan] = event_entry[4];
          //file_bin_.seekg(10, std::ios::cur);
          coinc++;
        } else
          file_bin_.seekg(-10, std::ios::cur);
      }

      one_spill.hits.push_back(one_event);
      count++;
    }

    spills_.pop_front();
  }
  PL_DBG << "coinc events " << coinc;
  return one_spill;
}


}
